...
public class PortfolioDetails extends BasePage{
	
	@FindBy(id=Constants.CREATE_PORTFOLIO_BUTTON)
	public WebElement createPortfolioButton;
	@FindBy(id=Constants.CREATE_PORTFOLIO_ID)
	public WebElement portFolioNameText;
	@FindBy(xpath=Constants.CREATE_PORT_BUTTON)
	public WebElement portFolioBut;
...
@FindBy(id=Constants.ADDSTOCK_BUTTON)
	...
	public PortfolioDetails(WebDriver dr) {
		super(dr);
	}
	...
	public void createPortFolio(String portName) {
		...
		boolean result=isElementPresent(Constants.CREATE_PORTFOLIO_BUTTON,"id");
		if(!result)
			repFailure("Element not found "+Constants.CREATE_PORTFOLIO_BUTTON);
		createPortfolioButton.click();
		portFolioNameText.clear();
		portFolioNameText.sendKeys(portName);
		portFolioBut.click();
